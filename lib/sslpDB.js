...
var sslpDB = /** @class */ (function() {
    ...
    sslpDB.prototype.findDocumentHash = async function(collection, query, field, sort) {
        ...
        try {
            ...
            if (fieldcat) {
                ...
                var hashcat = crypto.createHash('sha256').update(fieldcat).digest('hex');
            } else {
                //var hashcat = SparkMD5.hash('');
                var hashcat = crypto.createHash('sha256').update('').digest('hex');
            }
    ...
    sslpDB.prototype.createJournalEntry = async function(txid, blockId, blockHeight, timestamp, timestampUnix, action, collectionName, fieldData, recordData) {
        ...
        try {
			...
			var recordHash = crypto.createHash('sha256').update(action + '' + JSON.stringify(fieldData) + '' + JSON.stringify(recordData)).digest('hex');
            ...
            if (autoIndex == 1) // This is the first entry
            {
                ...
                var chainHash = crypto.createHash('sha256').update(docs.recordHash + '' + recordHash).digest('hex');
